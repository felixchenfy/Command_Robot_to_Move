...
def setup_yolo_files(args):
    ...
    print("Split all all images into [valid/train={:.2f}/{:.2f}]:".format(r, 1-r))
...
def parse_args():
    ...
    parser.add_argument("--config_file", type=str, default="config/config.yaml", help="path to config file. e.g.: ROOT/config/config.yaml")
    parser.add_argument("--verify_mask", type=MyBool, default=False, help="Whether write masked template images to file for user to verify if mask is correct or not")
...
def main(args):
    ...
    ALL_ON = True # Turn on all functions. For debug only.
