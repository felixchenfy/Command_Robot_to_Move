...
def put_text_with_background_color(
    ...
    cv2.putText(img, text, (x, y), font, fontScale=font_scale, color=(0, 0, 0), thickness=thickness)
...
class Yolo_Detection_Plotter_by_very_slow_plt(object):
    def __init__(self, args, IF_SHOW):
    ...
    def _plot(self, img, detections, classes):
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                print("\t+ Label: %s, Conf: %.5f" % (classes[int(cls_pred)], cls_conf.item()), end='; ')
...
class Yolo_Detection_Plotter_by_cv2(object):
    def __init__(self, args, IF_SHOW=True, 
    ...
    def savefig(self, filename):
        cv2.imwrite(filename, (self.img_disp*255).astype(np.uint8) )
    ...
    def plot(self, img, detections, classes):
        ...
        self._plot_onto_image(self.img_disp, detections, classes)
    ...
    def _plot_onto_image(self, img, detections, classes):
        args = self.args 
        colors = self.colors
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                print(f"box: center=({xc}, {yc}), w={width}, h={height}")
